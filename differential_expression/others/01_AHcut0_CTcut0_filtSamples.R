################################################################################
# Name: 01_AHcut0_CTcut0_allSamples                                            #
...
mat.gse.ahctCut0 <- mat_gse_filt

colnames(mat.gse.ahctCut0$counts)

mat.gse.ahctCut0$abundance <- mat.gse.ahctCut0$abundance[idx,c(1:3,16:19)]
mat.gse.ahctCut0$counts <- mat.gse.ahctCut0$counts[idx,c(1:3,16:19)]
mat.gse.ahctCut0$length <- mat.gse.ahctCut0$length[idx,c(1:3,16:19)]

colnames(mat.gse.ahctCut0$counts)

coldata.ahct <- coldata[coldata$names %in% colnames(mat.gse.ahctCut0$counts),1:2]
...
write.csv(res.padj05.lfc0, file = "filtSamples/AHcut0_CTcut0_res05_sig_fc0.csv")
################################################################################
# plot
library(ggplot2)
library(ggrepel)
