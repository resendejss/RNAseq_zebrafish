################################################################################
# Name: 04_AHcut5_CTcut5_fisltSamples                                          #
...
mat.gse.ahctCut5 <- mat_gse_filt

colnames(mat.gse.ahctCut5$counts)

mat.gse.ahctCut5$abundance <- mat.gse.ahctCut5$abundance[idx,c(8:11,23:25)]
mat.gse.ahctCut5$counts <- mat.gse.ahctCut5$counts[idx,c(8:11,23:25)]
mat.gse.ahctCut5$length <- mat.gse.ahctCut5$length[idx,c(8:11,23:25)]

colnames(mat.gse.ahctCut5$counts)

coldata.ctah <- coldata[coldata$names %in% colnames(mat.gse.ahctCut5$counts),1:2]

ddsTxi <- DESeqDataSetFromTximport(mat.gse.ahctCut5,
                                   colData = coldata.ctah,
                                   design = ~ Trat_01)
...
write.csv(res.padj05.lfc0, file = "filtSamples/AHcut5_CTcut5_res05_sig_fc0.csv")
################################################################################
# plot
library(ggplot2)
library(ggrepel)
