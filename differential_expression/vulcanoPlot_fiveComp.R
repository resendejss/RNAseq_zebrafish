...
vulcanoPlot <- function(allFilt){
  ...
  files <- list.files(paste("five_comp/",allFilt,sep = ""))
  
  for (i in files) {
    data <- read.csv(paste("five_comp", allFilt, i, sep="/"))
    ...
    write.csv(data_upDown, paste("five_comp/", allFilt,"/tableVulcanoplot_",i, sep = ""))
    ...
    ggsave(paste("five_comp/", allFilt,"/vulcanoPlot_",gsub(".csv",".pdf",i),sep = ""),
